...
<!-- 			      <button type="reset" id="resetButton" class="layui-btn layui-btn-primary">重置</button> -->
	   			...
	   			<div class="layui-form-item">
					...
					<div class="layui-input-item layui-inline">
			           <input type="text" class="layui-input start-fee" name="kehuID" id="dingdanID">
			     	</div>
			     	<div class="layui-inline">
			     		<label class="layui-form-label layui-inline">客户名称:</label>
			     		<div class="layui-input-item layui-inline">
    ...
    layui.use(['element','table','jquery', 'form','laydate', 'layer'], function(){
		...
		// ,url:'/demo/table/user/'
		    ...
		    ,cols: [[	        
		      ...
		      , {field:'客户ID', width:'10%', title: '客户ID'}
		      , {field:'客户名称', width:'20%', title: '客户名称'}
		      , {field:'电话', width:'20%', title: '电话'}
		      , {field:'邮箱', width:'20%', title: '邮箱'}
		      , {field:'地址', width:'20%', title: '地址'}
		    ]],
		    data:AddData
		  });
        ...
        form.on('submit(selectAllKeHu)', function(data){
            $.ajax({
                type : 'post',
                url : '${pageContext.request.contextPath}/selectKeHu.selectAllKeHu',
               ...
               // data :  JSON.stringify(da),
                success : function(data) {
                	...
                	for(i = data.length-1; i >=0; i--) {
                		var da = [
                        	{No:i, 客户ID:"",客户名称:"",电话:"",邮箱:"",地址:""},
           		         ];
                		da["No"] = i+1;
                		da["客户ID"] = changeToCode(data[i][0]);
                		da["客户名称"] = data[i][1];
                		da["电话"] = data[i][2];
                		da["邮箱"] = data[i][3];
                		da["地址"] = data[i][4];
      ...
      //提交，刷新表单
        form.on('submit(selectKeHu)', function(data){
        	if(data.field.kehuID != null && data.field.kehuID.length > 7) {
        		layer.alert("客户ID格式不合法！");
        		return false;
        	}
            ...
            $.ajax({
                type : 'post',
                url : '${pageContext.request.contextPath}/selectKeHu.selectKeHu',
                //设置contentType类型为json
                contentType : 'application/json;charset=utf-8',
                //json数据
                data :  JSON.stringify(da),
