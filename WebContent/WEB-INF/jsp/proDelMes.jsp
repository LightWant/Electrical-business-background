	...
	<div class="layui-layout layui-layout-admin">
		...
		<div class="layui-body" >
    		...
    		<form class="layui-form" action="" >
    			<div class="layui-form-item">
    				<label class="layui-form-label layui-inline">销毁批次编号:</label>
					<div class="layui-input-item layui-inline">
			           <input type="text" class="layui-input" name="xhpc" id="xhpc">
			     	</div>
			     	<button type="button" class="layui-btn" lay-submit lay-filter="formDemo">查询该销毁批次的信息</button>
...
//表格数据缓存
    ...
    layui.use(['element','table','jquery', 'form','laydate', 'layer'], function(){
		...
		// ,url:'/demo/table/user/'
		    ...
		    ,cols: [[	   
		    	//销毁批次,产品ID,产品名称,入库批次ID,销毁数量,销毁时间,销毁负责人ID
		      ,	{field:'销毁批次',  width:'10%', title: '销毁批次'}
		      , {field:'产品ID', width:'10%', title: '产品ID'}
		      , {field:'产品名称', width:'16%', title: '产品名称'}
        ...
        form.on('submit(formDemo)', function(data){
          //  console.log(data.field);
          	var dd = data.field.xhpc
          	if(dd == ""||dd == null){
          		alert("请先输入查询的销毁批次！");
          		return false;
          	}
            var da = {
            	name:data.field.xhpc
            };
            $.ajax({
                type : 'post',
                url : '${pageContext.request.contextPath}/Product_Staff.selXhInfo',
                ...
                success : function(data) {
                	...
                	if(data!=null){
	                	var get = data.split(",");
	                	var da = {
	                			销毁批次:get[0],产品ID:changeToCode(get[1]),产品名称:get[2],入库批次ID:get[3],销毁数量:get[4],销毁时间:get[5],销毁负责人ID:changeToCode(get[6]),销毁仓库ID:get[7]
	                	};
        ...
        form.on('submit(formDemo1)', function(data){
            ...
            $.ajax({
                type : 'post',
                url : '${pageContext.request.contextPath}/Product_Staff.selXhViaFzr',
                //设置contentType类型为json
                contentType : 'application/json;charset=utf-8',
                //json数据
                data :  JSON.stringify(da),
        ...
        form.on('submit(formDemo2)', function(data){
            ...
            $.ajax({
                type : 'post',
                url : '${pageContext.request.contextPath}/Product_Staff.selXhViaTime',
                //设置contentType类型为json
                contentType : 'application/json;charset=utf-8',
                //json数据
                data :  JSON.stringify(da),
